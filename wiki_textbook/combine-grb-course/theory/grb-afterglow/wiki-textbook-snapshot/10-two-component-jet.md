...
Structured jet 章节把喷流写成连续的 angular profile `\mathcal{E}(θ)=dE/dΩ` 和 `Γ(θ)`。Two-component jet 是这个图像的离散化版本：一个窄而快的 narrow component 嵌在一个宽而慢的 wide component 中。它的价值不只是“多加一个光变成分”，而是把 prompt brightness、early afterglow break、late bump / resurgence、radio calorimetry 和 jet-break masking 放进同一个几何-动力学框架。
...
```tex
\mathcal{E}(θ)=\frac{dE}{dΩ}(θ)=
\begin{cases}
\mathcal{E}_n, & 0\le θ<θ_n,\\
\mathcal{E}_w, & θ_n\le θ<θ_w,\\
0, & θ\ge θ_w,
\end{cases}
```
...
```tex
\mathcal{E}=\mathcal{E}(θ),\quad Γ_0=Γ_0(θ),
```

而 two-component jet 把它近似成

```tex
(\mathcal{E}_n,Γ_{0,n},θ_n) + (\mathcal{E}_w,Γ_{0,w},θ_w).
```
