...
| formula ID | 内容 | Python reference | 验证 |
| --- | --- | --- | --- |
| `SSC-NU-IC-001` | Thomson IC characteristic frequency | `core/radiation/ssc.py::ssc_characteristic_frequency` | `radiation_mechanism_library_check.md` |
| `SSC-ETA-001` | slow/fast cooling radiative efficiency eta | `core/radiation/ssc.py::slow_cooling_radiative_efficiency` | `radiation_mechanism_library_check.md` |
| `SSC-Y-ITER-001` | toy self-consistent `Y` and `nu_c` iteration | `core/radiation/ssc.py::iterate_compton_y_and_cooling_frequency` | `radiation_mechanism_library_check.md` |
| `SSC-NU-M-IC-001` | `nu_m^IC` | `core/radiation/ssc.py::ssc_breaks_with_cooling_feedback` | `radiation_mechanism_library_check.md` |
| `SSC-NU-C-IC-001` | `nu_c^IC` with IC-corrected `nu_c` | `core/radiation/ssc.py::ssc_breaks_with_cooling_feedback` | `radiation_mechanism_library_check.md` |
| `GG-TAU-INTERNAL-001` | internal gamma-gamma opacity toy | `core/radiation/gamma_gamma.py::internal_gamma_gamma_opacity` | `radiation_mechanism_library_check.md` |
| `GG-EBL-ATTEN-001` | EBL attenuation from supplied `tau_EBL` | `core/radiation/gamma_gamma.py::ebl_attenuation_factor` | `radiation_mechanism_library_check.md` |

这些接口只提供 sanity / event-trend 层级。`tau_EBL` 必须来自外部 EBL model 或论文表格；当前代码不内置任何 EBL model。
