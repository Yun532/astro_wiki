...
本扩展层也不替换 `astro_wiki_cn` 原有 agent / workflow。它只作为 GRB 数据、模型、理论和复现的补充规则，在独立分支、独立任务和明确页面范围内运行；对原知识库已有简洁内容只做必要补充、链接和局部修正。
...
运行边界：这些 agent 规则是 GRB 扩展层的独立分工，不接管、不替换原项目已有 agent。实际并行运行时，应在当前任务/分支内单独启动，明确输入、输出和写入范围；不复用或修改原项目正在运行的 agent 配置，除非用户明确要求接入。
