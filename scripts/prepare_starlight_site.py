#!/usr/bin/env python3
"""Mirror wiki/ into an Astro Starlight content directory. Does not modify wiki/."""
from pathlib import Path
import shutil, re
ROOT=Path(__file__).resolve().parents[1]
BASE_PATH='/astro_wiki'
WIKI=ROOT/'wiki'; SITE=ROOT/'site'; DOCS=SITE/'src'/'content'/'docs'; PUBLIC=SITE/'public'; FIGPUB=PUBLIC/'figures'
...
def link_for(target):
    rel=target.relative_to(WIKI).with_suffix('').as_posix()
...
def convert_links(txt,src_file):
    ...
    def markdown_repl(m):
        ...
        try:
            if target.exists() and WIKI.resolve() in target.parents:
...
def convert_images(txt,src_file):
    def repl(m):
        ...
        try:
            if img.exists() and WIKI.resolve() in img.parents:
                rel=img.relative_to(WIKI).as_posix(); out=FIGPUB/rel; out.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(img,out)
...
count=0
for p in files:
    out=DOCS/p.relative_to(WIKI); out.parent.mkdir(parents=True,exist_ok=True)
